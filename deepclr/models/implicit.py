...
class IMPLICITModule(nn.Module, metaclass=abc.ABCMeta):
    """Abstract base class for IMPLICIT modules."""
    def __init__(self):
        super().__init__()
...
class PositionalEncodingFeature(IMPLICITModule):
...
class SetAbstraction(IMPLICITModule):
    ...
    def forward(self, clouds: torch.Tensor, *_args: Any) -> torch.Tensor:
        ...
        merged_clouds = merge_features(xyz, features)
        print("clouds : ", clouds.shape)

        output_dict = {
            "original_cloud": clouds,
            "merged_cloud": merged_clouds,
        } 
        # return clouds
        return output_dict
...
class MotionEmbedding(IMPLICITModule):
    ...
    def forward(self, clouds_dict: Dict) -> torch.Tensor:
        ...
        merged_clouds = clouds_dict['merged_cloud']
        batch_dim = int(merged_clouds.shape[0] / 2)

        embed_clouds = self._embedding(merged_clouds[:batch_dim, ...],
                               merged_clouds[batch_dim:, ...])
        clouds_dict_buf['embed_clouds'] = embed_clouds
...
class Transformer(IMPLICITModule):
...
class OutputSimple(IMPLICITModule):
    ...
    def forward(self, cloud_dict: Dict) -> torch.Tensor:
        ...
        print(cloud_dict_buf)
        x = cloud_dict['embed_clouds']
...
class IMPLICITLoss(IMPLICITModule, metaclass=abc.ABCMeta):
...
class TransformLoss(IMPLICITLoss):
...
class TransformUncertaintyLoss(IMPLICITLoss):
...
class AccumulatedLoss(IMPLICITLoss):
...
def init_module(cfg: Config, *args: Any, **kwargs: Any) -> IMPLICITModule:
    """Initialize IMPLICITModule from config."""
    return factory(IMPLICITModule, cfg.name, *args, **cfg.params, **kwargs)


def init_loss_module(cfg: Config, label_type: LabelType, *args: Any, **kwargs: Any) -> IMPLICITLoss:
    """Initialize IMPLICITLoss from config."""
    return factory(IMPLICITLoss, cfg.name, *args, label_type=label_type, **cfg.params, **kwargs)


def init_optional_module(cfg: Optional[Config], *args: Any, **kwargs: Any) -> Optional[IMPLICITModule]:
    """Initialize optional IMPLICITModule from config."""
    if cfg is None:
        return None
    else:
        return factory(IMPLICITModule, cfg.name, *args, **cfg.params, **kwargs)
...
class IMPLICIT(BaseModel):
    ...
    _loss_layer: Optional[IMPLICITLoss]
