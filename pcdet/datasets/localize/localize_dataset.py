...
from ..odometry_dataset import OdometryDatasetTemplate


class LocalizeDataset(OdometryDatasetTemplate):
...
def create_localize_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    dataset = LocalizeDataset(
    ...
    train_filename = save_path / ('localize_infos_%s.pkl' % train_split)
    val_filename = save_path / ('localize_infos_%s.pkl' % val_split)

    print('------------------------Start to generate data infos------------------------')

    dataset.set_split(train_split)
    localize_infos_train = dataset.get_infos(
        class_names, num_workers=workers, has_label=True, num_features=num_features
    )
    with open(train_filename, 'wb') as f:
        pickle.dump(localize_infos_train, f)
    print('Custom info train file is save to %s' % train_filename)

    dataset.set_split(val_split)
    localize_infos_val = dataset.get_infos(
        class_names, num_workers=workers, has_label=True, num_features=num_features
    )
    with open(val_filename, 'wb') as f:
        pickle.dump(localize_infos_val, f)
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_localize_infos':
        import yaml
        ...
        create_localize_infos(
            dataset_cfg=dataset_cfg,
            class_names=['scan', 'map'],
            data_path=ROOT_DIR / 'data' / 'localize',
            save_path=ROOT_DIR / 'data' / 'localize',
        )
